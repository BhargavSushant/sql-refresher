...
SELECT c.first_name, c.last_name, c.email, c.phone, COUNT(o.order_id) AS total_orders, SUM(oi.quantity * oi.price_per_unit) AS total_spent
FROM Customers c
...
SELECT AVG(TIMESTAMPDIFF(DAY, o.order_date, o.shipped_date)) AS avg_processing_time
FROM Orders o
...
SELECT p.name, AVG(r.rating) AS avg_rating, COUNT(r.review_id) AS num_reviews
FROM Products p
...
ADD COLUMN commission_rate DECIMAL(5,2) NOT NULL DEFAULT 0.05;

SELECT 
  e.employee_id,  e.first_name,  e.last_name,  e.email,
  SUM(oi.price_per_unit* oi.quantity * (e.commission_rate / 100)) AS total_commission_earned
FROM 
  Employees e
